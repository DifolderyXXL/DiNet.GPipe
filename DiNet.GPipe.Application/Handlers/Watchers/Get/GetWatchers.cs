...
internal class GetWatchers(IWatcherManager manager) : IQueryHandler<GetWatchersQuery, List<WatcherResponse>>
{
    public async Task<Result<List<WatcherResponse>>> Handle(GetWatchersQuery query, CancellationToken ct)
    {
        ...
        return Result.Success(
            watchers.Select(
                watcher => new WatcherResponse(watcher.Id, watcher.ProjectName, watcher.GitUrl, watcher.Branches, watcher.Status))
            .ToList()
            );
